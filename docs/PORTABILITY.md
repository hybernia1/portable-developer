...
- spustit hashově ověřený Double Commander z `modules/filemanager/` s počátečními panely ve `www`, konfigurací pod `state/doublecmd` a portable dočasnými cestami.
...
Vestavěný terminál omezuje vlastní navigaci a volbu spustitelného souboru, ale nepředstavuje bezpečnostní sandbox Windows. Důvěryhodný PHP či Python program může používat běžná oprávnění uživatele a přistupovat i mimo portable kořen. Stejně tak Double Commander pouze startuje ve `www`; jako plnohodnotný uživatelský nástroj umožní vědomě přejít jinam. Tato hranice je v UI i dokumentaci explicitní.
