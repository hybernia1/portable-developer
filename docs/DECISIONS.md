...
## ADR-077 — Semantic WPF icon catalog

Keep neutral application icons as semantic WPF `Geometry` resources in one app-owned catalog and render them through a shared `AppIcon` control that inherits Fluent foreground and interaction context. Do not load a monolithic SVG sprite or add a runtime SVG library: WPF geometry keeps action and navigation icons dependency-free, theme-aware, and compatible with high contrast. Keep third-party technology identities as separately attributed SVG marks rendered by `BrandLogo` with their approved brand brushes. Pages use standard Fluent controls containing `AppIcon` plus localized text; icon-only actions remain limited to familiar tertiary commands with tooltips and automation names. Prevent drift with source and resource smoke tests rather than copying path presentation or concrete colors into feature views.

## ADR-078 — Native Fluent navigation selection

Use one selection grammar for the primary sidebar and secondary section navigation: a system Fluent control-fill surface identifies the selected item and a narrow system-accent indicator communicates route position without saturating the entire row. Continue to inherit the built-in `ListBoxItem` style and do not replace its control template, focus visual, keyboard behavior, or automation semantics. Keep scrolling bounded to the sidebar navigation collection so language and version controls remain stable, and align its native scrollbar with the shell edge. Project context stays in the shared header and uses semantic catalog icons; only actionable context failures render below the selector. This refines the visual presentation in ADR-069 and ADR-074 without changing the typed route or active-project ownership.

## ADR-079 — Project master/detail presentation

Keep Projects as a genuine selection-based master/detail workflow, but apply the same quiet Fluent selection resource and narrow accent indicator used by shell navigation. Selection identifies the project being inspected; the independent active badge identifies the tools context and must not move merely because the user changes list selection. Present the selected project as one bounded object surface with internal identity, command, capability, web, and management sections separated by dividers rather than nested cards. Use semantic icons for scanning while retaining text on every project command. Represent `Remove from list` with a non-destructive removal glyph rather than a trash glyph because the action preserves project files. This changes presentation only; stable project IDs, routed actions, portable catalog ownership, filesystem boundaries, and explicit activation remain unchanged.

## ADR-080 — Explorer-like file workspace

Keep the file manager as a genuine extended-selection workflow rather than converting it to either a generic `DataGrid` or a non-selectable catalog. Present its editable location, sortable details header, file collection, empty state, and paging footer as one bounded Explorer-like surface. Continue to inherit the standard Fluent `ListBoxItem` behavior and scope only the quiet selection brushes plus a narrow accent indicator to this file list. Preserve Name and Type in compact mode, collapse Size and Modified first, and prevent page-level horizontal scrolling. Render familiar navigation and file-kind symbols through the semantic `AppIcon` catalog, with localized tooltips and automation names for icon-only commands. This is a presentation change only: sorting, paging, rename, double-click navigation, context actions, drag and drop, clipboard transfer, project-root validation, and filesystem mutation remain owned by the existing view-model and service boundaries.

## ADR-081 — Shared native dialog composition

Use one shared, app-owned dialog composition for purpose headers, field labels, guidance, validation, and separated action footers while retaining native WPF `Window` chrome and standard Fluent controls. A semantic icon and one heading identify purpose; optional project identity is muted context rather than a second title. Compact dialogs remain fixed one-column modals with only Close in their native caption. Save/Create/Rename is the single accent action with icon and text, Cancel remains secondary, and warning or destructive meaning is communicated explicitly instead of recoloring the whole window. Long forms may own one body scrollbar but must keep the action footer outside it. Only reading workspaces such as scheduled-run output may resize and own bounded horizontal and vertical content scrolling. This changes presentation only: dialog results, fail-closed confirmation fallbacks, validation, Enter/Escape behavior, scheduler definitions, project configuration, conflict decisions, and all service ownership remain unchanged.

## ADR-082 — Shared managed-runtime header

Present Apache, MariaDB, and Selenium through one flat, app-owned runtime header containing recognizable technology identity, verified version, lifecycle state, concise context, local feedback, and an adaptive command slot. Separate the header from the page body with a quiet divider instead of treating status as another card. Use standard Fluent buttons: one accent start/stop action with semantic play/stop icon and neutral icon-and-text buttons for secondary commands. Apply this component only to independently controlled managed services; PHP remains a runtime-configuration surface and Composer, Node.js, and Python retain the shared package-manager composition. This is a presentation boundary only: existing routed handlers, process ownership, health checks, service guards, download behavior, and portable storage paths remain unchanged.

## ADR-083 — Shared native settings-form composition

Compose editable page sections through one app-owned `FormSection` that provides semantic identity, an optional description, body content, page-local feedback, and an optional separated action footer. Centralize only typography, spacing, help text, and primary/secondary command hierarchy; inputs remain standard WPF Fluent controls with visible labels, native focus, validation, and accessibility behavior. Related fields may use the existing adaptive split, but ordinary pages retain one outer vertical scroll owner and never add horizontal scrolling. Save or Create is the single accent action, while Reset and other secondary commands remain neutral. Keep `PasswordBox` controls that are read directly by secure page handlers in the page namescope rather than binding password text or moving it into a nested control. This changes presentation only: persistence timing, validation, password lifetime, service abstractions, database operations, and routed handlers remain unchanged.

## ADR-084 — Shared section identity and local adaptive breakpoint

Present informational and catalog sections through one app-owned `SectionHeader` containing a semantic icon, heading, optional description, optional metadata, and optional standard Fluent actions. The header owns layout only: it must remain inside the feature's coherent surface and must not own the body, collection, selection, scrolling, operation state, or command behavior. Extend `AdaptiveSplitPanel` with an opt-in local compact breakpoint so a nested header can stack within a narrow card even when the inherited workspace mode remains wide; the global workspace mode stays authoritative when no local breakpoint is specified. This changes presentation only: existing routed handlers, page models, service abstractions, portable state, and one-scroll-owner rules remain unchanged.

## ADR-085 — Restrained application accent and non-interactive focus surfaces

Retain the built-in system Fluent theme and its standard control templates, but use one app-owned Windows-blue accent for normal command fills, interaction states, progress, selection markers, and selected-text fills instead of inheriting an unrelated saturated desktop accent. Keep the palette in the central theme dictionary and remove only its framework-key overrides whenever Windows high contrast is active, allowing system accessibility colors to remain authoritative. This narrows the accent-ownership portion of ADR-069 and ADR-078 without introducing a custom control template or light/dark surface palette.

Suppress the focus adorner only on non-interactive page scroll containers and on file rows whose selected surface plus accent marker already communicates keyboard position. Buttons, inputs, selectors, navigation items, dialogs, and other interactive controls keep native focus behavior. In the file manager, reserve mouse double-click for opening and keep rename explicit through `F2` or the context menu; do not make a selected-name click compete with folder activation. Align the details header and rows inside one consistent inset while preserving extended selection, keyboard commands, drag/drop, paging, and service-owned filesystem safety. This refines ADR-080 presentation and interaction without changing file operations.

## ADR-086 — Concise steady-state interface copy

Treat page title, section identity, control labels, current values, concise lifecycle state, and explicit actions as sufficient explanation during an ordinary healthy state. Do not add descriptive paragraphs that merely repeat those visible facts, the application's general portability model, or implementation guarantees already documented elsewhere. Retain text when it changes a user's decision or enables recovery: validation constraints, unavailable dependencies, destructive consequences, security boundaries, failures, in-progress operations, and explicit action results remain visible and localized. Runtime headers hide their detail during normal ready/running/stopped states but reveal the existing diagnostic detail when a service fails or requires setup. This is a presentation-only refinement of ADR-082 through ADR-084; it does not remove service diagnostics, logs, validation, download verification, or portable-safety enforcement.

## ADR-087 — On-demand explanations and Windows-owned accent contrast

Keep recurring instructional copy out of steady-state forms when a user can understand and complete the task from the section identity, labels, values, and actions. Expose that optional context from a compact, keyboard-accessible information action in the shared section header and render it through one system-themed modal with localized content. Aggregate closely related explanation in that dialog, but keep validation, security boundaries that affect an immediate decision, destructive consequences, progress, failures, and recovery guidance visible beside the relevant action. Package installation therefore retains its trust/network warning while moving only the manager-specific directory explanation on demand; Selenium profile and cookie-vault creation move their general workflow descriptions on demand.

Do not override Windows accent fills or on-accent foreground brushes. Primary controls extend the framework `AccentButtonStyle` only for layout and inherit the system-selected accent together with its adaptive foreground and high-contrast behavior. Fixed colors remain limited to attributed technology marks. This supersedes the application-owned blue palette portion of ADR-085 and restores the accent ownership defined in ADR-069; ADR-085's focus and file-interaction decisions remain active.

Runtime theme changes must not leave application styles dependent on an implicit type-style lookup while WPF is replacing its Fluent resource dictionary. Standard-control extensions therefore derive from the framework's stable named `Default*Style` resources, and the typed page host uses one explicit app-owned style. Their colors remain dynamic Windows-owned resources, so accent, light/dark, and high-contrast changes still apply immediately without a self-referential style cycle.

Neutral application text and icons inherit their foreground and UI font through the Fluent visual context. Do not set a default foreground on `AppIcon`, because an icon inside an accent button must inherit that button's adaptive on-accent foreground. Explicit text brushes remain appropriate only for semantic hierarchy or status, and monospace families remain explicit only for paths, commands, code, and logs.

Keep the open sidebar on the same native window surface as the workspace instead of assigning it a separate layer fill. A 250-DIP open navigation measure gives labels and the native scrollbar Windows-like breathing room; retain only a quiet framework divider and preserve the established minimum workspace width by increasing the window minimum width with the pane.

Keep the sidebar scrollbar on the shell divider rather than inset inside the navigation rows. Continue to derive it from WPF's named Fluent scrollbar style and change only whole-control opacity: hidden while idle, softly discoverable while the navigation is hovered, and fully visible while directly hovered or dragged. Do not replace its framework template, system brushes, wheel, touchpad, keyboard, or automation behavior.

## ADR-088 — Lazy package inventories and bounded runtime verification cache

Treat the Composer, npm, and Python package lists as derived page state rather than a navigation precondition. Do not launch package-manager inventory commands during application startup or whenever a user returns to an already loaded page. A successful list result, including an empty result, remains authoritative for that page until the active project changes, the application installs or removes a dependency, or the user explicitly requests Refresh. A failed initial load remains eligible for retry. Keep the Refresh action because dependencies may also be changed through the portable terminal; do not persist derived inventory as user state or treat download caches as inventory caches.

Every operation that executes a portable tool must still pass runtime integrity checks. Reuse a successful SHA-256 result only within the current application process and only while the entrypoint length, creation time, last-write time, and expected digest remain unchanged. Any fingerprint change immediately recomputes and revalidates the entrypoint. This applies the existing module-verification cache policy to portable tool runtimes without weakening catalog pinning, process ownership, captured output, or portable path isolation.

## ADR-089 — Bounded transient action confirmations

Use one application-scoped transient notification host for short, noncritical confirmations whose result is already reflected by the interface, such as copying an identifier, saving a preference, or completing straightforward create, update, and remove actions. The latest confirmation replaces the previous one, remains visible for seven seconds, pauses while hovered, supports explicit dismissal, and announces new text through a polite WPF automation live region. It has no history and is not a Windows app notification.

Do not route validation, failures, warnings that affect a decision, restart requirements, dependency problems, long-running progress, or service lifecycle state through this host. Those remain durable and local to the field, form, runtime header, or operation card that owns the action. Successful refreshes and launches of external windows are silent when the resulting state is self-evident. This narrowly supersedes the completed-action portion of ADR-072 and the blanket toast exclusion in the WPF simplification plan; it does not restore a generic shell status channel or duplicate operation presentation.

## ADR-090 — Catalog-header creation dialogs

Keep a catalog page focused on its persisted objects. When a short create or import workflow exists only until the user commits it, expose one explicit Add/Create action in the catalog header and collect its transient inputs in an application-owned native modal. Selenium profile enrollment, cookie-vault import, and database creation follow this pattern. Their page models retain catalog rows, operation progress, and durable failures, but do not retain hidden draft names, browser choices, or selected import paths after a modal closes.

The modal performs immediate required-field validation and returns a bounded value to the existing application handler; domain and service validation remains authoritative. Security settings, service configuration, and other state that benefits from continuous visibility stays inline. This extends the shared dialog composition in ADR-081 and refines the page-state ownership in ADR-072 without moving browser, vault, database, filesystem, or process work into UI code.

## ADR-091 — Field-anchored validation with transient failure announcement

Permit the application transient host to carry a critical intent for a short form rejection only when the responsible native field is simultaneously focused, marked with the Windows critical brush, and given the same localized message through automation help. The notification supplies immediate and screen-reader-visible feedback; the field marker preserves the location of the problem until the user edits the value. Do not use this intent for long-running progress, service lifecycle, dependency state, or a failure that lacks a clear recovery target.

Apply this first to the MariaDB root-password form. Validate the shared 8–128-character policy before executing the account service, mark the password field for a policy violation, mark confirmation for a mismatch, and keep both secret values view-local. Successful changes continue to use the transient success intent. This narrowly supplements ADR-089 for actionable field validation without reviving runtime-header or generic shell status leakage.

## ADR-092 — Immutable scheduler run identity

Persist the scheduled command kind and project-relative target with every new history record. History describes what actually ran, so it must not depend on a task definition that may later be edited or removed. Keep the additive fields optional under the existing bounded history schema: older records remain readable and may borrow identity from a currently matching task for presentation, but they are never rewritten merely by being viewed.

Render run history with the same technology-led identity block and responsive card grammar as the task catalog, while keeping outcome, timestamp, duration, trigger, and log actions specific to the recorded run. Include the persisted type and target in filtering and details. This extends the scheduler history presentation in ADR-080 without changing execution, captured-output, retention, or atomic storage boundaries.
