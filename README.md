...
The project is free software under [GPL-3.0-or-later](LICENSE). Version **1.29.0** is a complete but currently unsigned release. Windows Smart App Control or SmartScreen may block it; do not disable Windows security to run the application. See the [Code signing policy](docs/CODE_SIGNING_POLICY.md).
...
2. Verify the executable:
   ...
   ```powershell
   Get-FileHash .\PortableDeveloper-win-x64-1.29.0.exe -Algorithm SHA256
   ```
...
```powershell
.\scripts\Publish-Online-Windows.ps1 -Version 1.29.0 -SingleExecutable
```
