...
The terminal parses a small allowlist of PHP, Composer, Python, filesystem, and service commands. Bundled runtime commands use an owned asynchronous process session with UTF-8 streams, incremental output, line-oriented standard input, a bounded display buffer, timeout, Ctrl+C termination, and kill-tree cleanup. Output is batched onto the WPF dispatcher so chatty tools cannot monopolize the UI thread. It still rejects shell operators and keeps its logical working directory under the active project; it does not expose a PTY, `cmd.exe`, or PowerShell. This is an accidental-damage boundary, not an OS sandbox. The file manager enforces the same project root, rejects link/reparse escapes, and opens files with the Windows association when available or the verified portable editor as a fallback. Directory listings are sorted and paged before reaching WPF; editable paths are canonicalized against the active root and large UI collections use bounded virtualized viewports.
