...
JRE 25.0.3, geckodriver 0.37.1, Composer 2.10.2, Python 3.13.0 s pip 24.2 a phpMyAdmin 5.2.3 jsou přibalené závislosti či nástroje evidované v `bundle-manifest.json`. Archiv geckodriveru pro Windows x64 se před rozbalením ověřuje připnutým SHA-256 a výsledný EXE se zapisuje do samostatného `drivers/bundled/drivers.json`. Composer a Python dostávají samostatná hashová metadata nástroje. phpMyAdmin se přebírá z připraveného zdrojového stromu, ověřuje pomocí hashů release markeru a `composer.lock` a balí se bez lokálního `config.inc.php`, adresáře `setup` a dočasných dat.
...
4. Apache, PHP, JRE, Composer, Python, MariaDB a Selenium se normalizují do `modules/`; geckodriver do `drivers/bundled/` a phpMyAdmin do `tools/phpmyadmin/`.
