...
public sealed class WorkspaceFileManagerPresentationTests
{
    ...
    public void File_manager_uses_type_aware_icons_with_native_context_actions()
    {
        ...
        Assert.Contains("<controls:AppIcon", fileManager, StringComparison.Ordinal);
        Assert.DoesNotContain("<Path", fileManager, StringComparison.Ordinal);
        Assert.Contains("ItemContainerStyle=\"{StaticResource FileListItemStyle}\"", fileManager, StringComparison.Ordinal);
        Assert.Contains("Style=\"{StaticResource DetailsHeaderButtonStyle}\"", fileManager, StringComparison.Ordinal);
        Assert.Contains("x:Key=\"ListBoxItemSelectedBackgroundThemeBrush\"", fileManager, StringComparison.Ordinal);
        Assert.Contains("{DynamicResource SubtleFillColorSecondary}", fileManager, StringComparison.Ordinal);
        Assert.Contains("{DynamicResource AccentFillColorDefaultBrush}", fileManager, StringComparison.Ordinal);
        Assert.Contains("Converter={StaticResource LayoutModeVisibilityConverter}, ConverterParameter=Wide", fileManager, StringComparison.Ordinal);
        Assert.Contains("AutomationProperties.Name=\"{Binding Text.RefreshFiles}\"", fileManager, StringComparison.Ordinal);
        Assert.Contains("Text=\"{Binding Text.ItemsPerPage}\"", fileManager, StringComparison.Ordinal);
        Assert.Contains("ScrollViewer.HorizontalScrollBarVisibility=\"Disabled\"", fileManager, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("WorkspaceName_PreviewMouseLeftButtonUp", fileManager, StringComparison.Ordinal);
        ...
        Assert.Contains("Grid.Row=\"1\" Margin=\"10,8,10,0\"", fileManager, StringComparison.Ordinal);
        Assert.Contains("Grid.Row=\"2\" Margin=\"10,4,10,8\"", fileManager, StringComparison.Ordinal);
        Assert.Contains("e.ClickCount != 2", File.ReadAllText(Path.Combine(repositoryRoot, "src", "PortableDeveloper.App", "MainWindow.FileManager.cs")), StringComparison.Ordinal);
        Assert.Contains("case Key.F2", File.ReadAllText(Path.Combine(repositoryRoot, "src", "PortableDeveloper.App", "MainWindow.FileManager.cs")), StringComparison.Ordinal);
