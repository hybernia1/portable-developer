...
Selenium controller používá výhradně katalogově ověřený `selenium-server.jar` a explicitní `modules/jre/<verze>/bin/java.exe`. Balíček Selenium instaluje Server a zmenšený Microsoft OpenJDK. Browser se instaluje vždy jako celý spravovaný pár: Mozilla Firefox + geckodriver nebo Chrome for Testing + ChromeDriver. Samotný Grid nic nestahuje, Selenium Manager je vypnutý a systémový `PATH` se nepoužívá.

Katalogové drivery se ukládají pod `drivers/bundled/<browser>/<verze>/`, zapisují do společného `drivers.json` a inventář při každém načtení ověří jejich SHA-256. Browsery leží pod `modules/browsers/<browser>/<verze>/`. `SeleniumBrowserEnvironmentInventory` přijme jen přesnou katalogovou dvojici ve stavu Ready; uživatelský ani systémový driver s ní nespáruje. Grid stereotype obsahuje explicitní browser binárku i driver. Aplikace neskenuje instalace Edge, Chrome ani Firefoxu ve Windows a nečte jejich profily.

Master profily vznikají výhradně pod `temp/selenium-profile-creation/<token>` ve spravovaném browseru. Aplikace otevře jeho stránku správy účtu, uživatel se přihlásí a browser zavře; cílová webová URL se při zakládání profilu nezadává. Kopírování, normalizace, hashování a úklid běží mimo UI vlákno a formulář průběžně ukazuje aktuální fázi. Servisní vrstva odmítne zdroj mimo vyhrazený portable prostor. Výsledek se uloží pod `profiles/selenium/<id>/master` pouze ke čtení. Sidecar manifest eviduje browser, verzi, layout, počet a velikost souborů a SHA-256 každého souboru. C# jej ověří při načtení a vlastní Node znovu před relací. Capability `portable:profile=<id>` vytvoří kopii pod `temp/selenium-profiles/<token>`; Chromium dostane `--user-data-dir`, `--profile-directory` a `--disable-sync`, Firefox vlastní `-profile` s vypnutým Firefox Account Sync. Kopie se smaže při neúspěšném startu, WebDriver DELETE, Node stopu nebo periodickém zjištění zaniklé relace; osiřelé kopie po pádu se odstraní při příštím startu aplikace.

Firefox je doporučený pro přenos celého přihlašovacího profilu; Chromium může i v app-managed profilu používat ochrany navázané na účet nebo počítač. Pro přenos pouze explicitně exportovaných cookies slouží samostatný vault pod `profiles/selenium-vaults/<id>`. Import normalizuje povolená pole, zahodí prošlé a duplicitní položky a šifruje payload pomocí AES-256-GCM s automaticky vytvořeným 256bitovým klíčem pod `state/selenium-cookie-vault.key`. `portable:vault=<id>` nechá vlastní Java Node payload autentizovat a rozšifrovat přímo v paměti, vložit po jednotlivých doménách a relaci dál provozovat v čistém browser profilu. Čitelný dočasný soubor nevzniká; klíč je přenositelný spolu s aplikací, a proto nechrání při odcizení celé složky.

Počet souběžných relací, Selenium `session-timeout` a výchozí zákaz stahování se ukládají do `state/selenium-settings.json`; port pochází z centrálního `state/port-settings.json`. Každý projekt automaticky vlastní `seldownloads` ve svém neveřejném projektovém prostoru. Po povolení před startem serveru Node přepíše klientské download preferences a Chromium policy vynutí také přes CDP; pokus klienta policy změnit odmítne. Soubory zůstávají mezi účty i relacemi, ale změna aktivního projektu je při běžícím Selenium blokovaná. U legacy Default projektu Apache přístup do `seldownloads` výslovně zakáže. Timeout představuje maximální neaktivitu, nikoli absolutní dobu běhu. Běžící relace UI načítá z lokálního GraphQL endpointu a ukončuje standardním `DELETE /session/{id}` až po potvrzení uživatele. Základní distribuce neobsahuje browser; oba spravované balíčky se stáhnou pouze po explicitní akci.
