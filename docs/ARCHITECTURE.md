...
Selenium controller používá výhradně katalogově ověřený `selenium-server.jar` a explicitní `modules/jre/<verze>/bin/java.exe`. Balíček Selenium instaluje pouze Server a zmenšený Microsoft OpenJDK. Žádný WebDriver není výchozí: EdgeDriver, ChromeDriver a geckodriver mají vlastní karty v Selenium katalogu a instalují se jen po explicitní akci. Samotný běžící Selenium Server nic nestahuje, Selenium Manager je vypnutý a systémový `PATH` se nepoužívá.

Katalogové drivery se ukládají pod `drivers/bundled/<browser>/<verze>/`, zapisují do společného `drivers.json` a inventář při každém načtení ověří jejich SHA-256. Uživatel může do `drivers/custom/` vložit standardně pojmenovaný `geckodriver.exe`, `chromedriver.exe` nebo `msedgedriver.exe`; inventář ignoruje reparse points a použije explicitní cestu uvnitř portable kořene. Vlastní driver je uživatelský spustitelný kód a UI jej proto odlišuje od ověřeného katalogového driveru. Chrome a Edge vyžadují driver odpovídající verzi hostitelského prohlížeče; aplikace hostitelský browser nemění ani neaktualizuje.

Master profily jsou importované pod `profiles/selenium/<id>/master`, po importu se označí pouze ke čtení a zdrojová složka mimo aplikaci se dál nepoužívá. Vlastní Node rozšíření se při startu sestaví z přibaleného zdroje proti přesné nainstalované verzi Selenium a načte přes `--ext`. Požadavek s namespaced capability `portable:profile=<id>` vytvoří kopii pod `temp/selenium-profiles/<token>` a vloží ji do `goog:chromeOptions`, `ms:edgeOptions` nebo `moz:firefoxOptions`. Kopie se smaže při neúspěšném startu, WebDriver DELETE, Node stopu nebo periodickém zjištění zaniklé relace; osiřelé kopie po pádu se odstraní při příštím startu aplikace.
